...
const VERSION = "G6-POLICY-RANKING-2.1-HANDOFF-R1";
...
function districtRecord(year, district, band, sex) {
  return districtRecords.find((row) => row.year === year && districtName(row.geography) === districtName(district) && row.ageBand === band && row.sex === sex);
}

function baselinePopulation(district, band, sex) {
  return baselineRecords.find((row) => districtName(row.geography) === districtName(district) && row.ageBand === band && row.sex === sex)?.population ?? null;
}
