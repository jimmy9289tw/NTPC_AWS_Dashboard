...
def find_population_snapshots(years: tuple[int, ...]) -> dict[int, Path]:
    ...
    for year in years:
        ...
        current_run = [item for item in matches if item.parent.name == f'annual-refresh-{year}']
        selected[year] = current_run[0] if len(current_run) == 1 else max(matches, key=lambda item: item.stat().st_mtime)
    return selected
...
def categorical_stream(path):
    if path.suffix.lower() == '.csv':
        ...
        with zipfile.ZipFile(path) as archive:
            ...
            if len(names) != 1:
                raise ValueError('官方壓縮檔應只有一份 CSV')
...
def read_categorical_zip(
    ...
    *,
    ...
    rows_read = 0
    rows_ntpc = 0
    with categorical_stream(path) as text:
        if True:
...
def build_registered_csv(package: Path, years: tuple[int, ...]) -> tuple[Path, dict]:
    ...
    for year in years:
        ...
        # S3 不存 ZIP；使用同一官方檔案逐列保留的新北市原始 CSV。
        # 僅改輸入介面，以下 PCLM、IPF、分母與公式保持不變。
        if education_path.with_suffix('.csv').exists():
            education_path = education_path.with_suffix('.csv')
        if marital_path.with_suffix('.csv').exists():
            marital_path = marital_path.with_suffix('.csv')
