...
PACKAGE = ROOT / "deliverables" / "NTPC_Youth_18-35_Government_Open_Data_Package_V2.2_20260825"
...
def main() -> None:
    failures: list[str] = []
    docs = sorted(PACKAGE.rglob("*V2.2.docx"))
    if len(docs) != 8:
        failures.append(f"docx_count={len(docs)}")
    texts = {path.name: docx_text(path) for path in docs}
    corpus = "\n".join(texts.values())
    for forbidden in ("V2.0", "3.76", "MOI-POP1Y-current.csv", "把年度值複製12次後標為官方"):
        if forbidden in corpus:
            failures.append(f"forbidden_text:{forbidden}")
    required = {
        "00_資料包總覽_治理與使用說明_V2.2.docx": ("PERIOD_TO_PERIOD", "RATIO_OF_SUMS", "NO_TEMPORAL_ESTIMATE"),
        "01_年齡人口結構_處理方法與驗證範例_V2.2.docx": ("OpenAPI", "7,752", "7,781", "845,938", "LAST_DEC"),
        "02_失業率_處理方法與驗證範例_V2.2.docx": ("6.4%", "6.52%", "14÷229", "半年平均", "Chow-Lin"),
        "04_教育程度_處理方法與驗證範例_V2.2.docx": ("臺灣地區", "只有兩組邊際", "IPF", "SOURCE_ANNUAL_NATIVE"),
        "07_整合查核與AWS_Kiro流程_V2.2.docx": ("source_period", "retrieved_at", "EventBridge", "RATIO_OF_SUMS", "v_youth_metric_monthly"),
    }
    ...
    integration_path = PACKAGE / "07_整合與查核" / "reference" / "青年18-35歲資料整合表_V1.6.csv"
    ...
    if len(integration) != 77 or len(set(ids)) != 77:
    ...
    temporal_required = {
        ...
        "monthly_availability", "annual_aggregation_method", "comparison_modes_allowed",
        "comparison_warning", "temporal_method_code", "temporal_estimation_status",
    }
    if not integration or not temporal_required.issubset(integration[0].keys()):
        failures.append("temporal_fields_missing")
    expected_records = {
        "AGEINT-073": ("6.4", "YEAR", "ANNUAL_AVERAGE"),
        "AGEINT-074": ("6.0", "HALF_YEAR", "HALF_YEAR_AVERAGE"),
        "AGEINT-075": ("229", "HALF_YEAR", "HALF_YEAR_AVERAGE"),
        "AGEINT-076": ("14", "HALF_YEAR", "HALF_YEAR_AVERAGE"),
        "AGEINT-077": ("6.113537", "HALF_YEAR", "HALF_YEAR_AVERAGE"),
    }
    by_id = {row["record_id"]: row for row in integration}
    for record_id, (value, granularity, basis) in expected_records.items():
        row = by_id.get(record_id)
        if row is None or row["adjusted_value"] != value or row["time_granularity"] != granularity or row["period_basis"] != basis:
            failures.append(f"temporal_record:{record_id}")
    if not (PACKAGE / "00_總覽與治理" / "time_granularity_policy.csv").exists():
        failures.append("time_policy_missing")
    if not (PACKAGE / "07_整合與查核" / "automation" / "config" / "time-granularity-policy.json").exists():
        failures.append("time_policy_json_missing")
