...
def publish(files,metadata):
    ...
    s3.put_object(Bucket=bucket,Key=pointer_key,Body=json.dumps(pointer).encode(),ServerSideEncryption='AES256',ExpectedBucketOwner=account,**({'IfMatch':old_etag} if old_etag else {'IfNoneMatch':'*'}))
    return pointer
def main():
