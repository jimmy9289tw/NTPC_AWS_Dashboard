...
def run(store,fetch=request,limit_new=12):
    ...
    try:
        ...
        if changed or not previous or getattr(store,'review_pending',lambda:False)():
            ...
            pointer=publish(store,'monthly',payloads,{'source_period_max':latest,'period_count':len(desired),'rows':len(records),'checked_at':checked_at,'review_run_id':run_id,'annual_window_unchanged':'110–114'},etag)
